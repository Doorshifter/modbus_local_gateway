...
from ..const import (
    MAX_READ_SIZE_DEFAULT,
    MAX_REGISTERS_PER_SECOND_DEFAULT,
)
...
class ModbusDeviceInfo:
    ...
    def __init__(self, fname: str, hass=None) -> None:
        """Initialise the device config"""
        self.fname: str = fname  # just the filename

        # Ensure we use hass.config.path when available
        if hass is not None:
            filename: str = hass.config.path(
                f"custom_components/modbus_local_gateway/device_configs/{fname}"
            )
        else:
            # fallback: use CONFIG_DIR (legacy)
            from ..device_configs import CONFIG_DIR
            filename: str = join(CONFIG_DIR, fname)
        _LOGGER.debug("Loading device YAML config from: %s", filename)
    ...
    @property
    def max_registers_per_second(self) -> int:
        """Maximum number of registers per second the device can handle"""
        if self._config and isinstance(self._config, dict) and DEVICE in self._config:
            return self._config[DEVICE].get(MAX_REGISTERS_PER_SECOND, MAX_REGISTERS_PER_SECOND_DEFAULT)
        return MAX_REGISTERS_PER_SECOND_DEFAULT
    ...
    def max_read_size(self) -> int:
        """Maximum number of registers to read in a single request"""
        if self._config and isinstance(self._config, dict) and DEVICE in self._config:
            value = self._config[DEVICE].get(MAX_READ, MAX_READ_SIZE_DEFAULT)
            # treat 0 or None as default, never allow <1
            if value is None or value == "" or int(value) < 1:
                return MAX_READ_DEFAULT
            return int(value)
        raise DeviceConfigError()

    @property
    def max_registers_per_second(self) -> int | None:
        """
        Maximum number of registers per second the device can handle.
        If missing/blank/0/negative, return None to signal 'RTT-only'.

        If the key is not present (val is None): returns None → unlimited/auto.
        If the value is an empty string (val == ""): returns None → unlimited/auto.
        If the value, when cast to int, is 0 or negative (int(val) <= 0): returns None → unlimited/auto.
        
        """
        if self._config and isinstance(self._config, dict) and DEVICE in self._config:
            val = self._config[DEVICE].get(MAX_REGISTERS_PER_SECOND, None)
            if val is None or val == "" or int(val) <= 0:
                # None means unlimited, use RTT only
                return None
            return int(val)
        return None
