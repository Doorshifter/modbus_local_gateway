...
class Conversion:
    ...
    def _convert_from_register_response(
        ...
        """Convert from register response or directly from a register sequence"""
        from collections.abc import Sequence as abcSequence

        # Accept a list/tuple/Sequence[int] or a modbus response
        ...
        elif isinstance(response, abcSequence) and not isinstance(response, str):
            # Already a register sequence
            registers = self._swap_registers(list(response), desc)
        else:
            _LOGGER.error("Invalid response type in _convert_from_register_response: %r (%s)", response, type(response))
